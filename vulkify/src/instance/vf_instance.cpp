...
vk::PresentModeKHR select_mode(VKGpu const& gpu, std::span<VSync const> desired) {
...
refactor::GfxDevice const& VulkifyInstance::gfx_device() const {
	static auto const s_inactive = refactor::GfxDevice{};
	return s_inactive;
}
...
refactor::GfxDevice const& HeadlessInstance::gfx_device() const {
	static auto const s_inactive = refactor::GfxDevice{};
	return s_inactive;
}
...
vk::PresentModeKHR select_mode(GpuInfo const& gpu, std::span<VSync const> desired) {
	for (auto const vsync : desired) {
		if (gpu.gpu.present_modes.test(vsync)) { return refactor::from_vsync(vsync); }
	}
	return vk::PresentModeKHR::eFifo;
}
} // namespace
...
Vram const& VulkifyInstance::vram() const { return g_inactive.vram; }
