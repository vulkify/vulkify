...
void Sprite::draw(refactor::Surface const& surface, RenderState const& state) const {
	if (m_texture) {
		...
		surface.draw(refactor::Drawable{instance, m_buffer.handle()}, state);
	}
}
...
auto Sprite::Sheet::set_uvs(ktl::not_null<refactor::Texture const*> texture, std::uint32_t const rows, std::uint32_t const columns, glm::uvec2 const pad)
	-> Sheet& {
...
auto Sprite::Sheet::set_texture(refactor::Handle<refactor::Texture> texture) -> Sheet& {
	m_texture = texture;
	return *this;
}
} // namespace vf
